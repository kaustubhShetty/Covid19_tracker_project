      ...
      <table class="table table-striped" style="cursor: pointer;">
        ...
        <tbody>
          <tr v-bind:key="ele.state" v-for="ele in newJsonListOfDictionaries">
            <td v-on:click="getStateName(ele.state)">{{ stateAcronymToFullName[ele.state] }}</td>
            <td>{{ ele.confirmed }}</td>
            <td>{{ ele.recovered }}</td>
          </tr>
        </tbody>
...
export default {
  ...
  mounted() {
    axios.get("/api/states_daily.json")

        .then((response) => {
            console.log("API Response:", response.data);  // Logs the raw API response
            this.originalJsonData = response.data;
            this.prepareData();  // Function to process data
        })
        .catch(error => {
            console.error("API Fetch Error:", error);  // Logs error if API call fails
        });
},
methods: {
    prepareData() {
        // Assuming you process your data here to make it suitable for rendering
        this.newJsonListOfDictionaries = this.originalJsonData.states_daily.map(item => ({
            state: item.state,  // Assuming 'state' is a key you need
            confirmed: item.confirmed,
            recovered: item.recovered
        }));
        console.log("Processed Data for Table:", this.newJsonListOfDictionaries);  // Check processed data
    }
},
